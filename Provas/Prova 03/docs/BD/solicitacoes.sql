...
create view vw_solicita as
select s.Num_Sol, s.Data_sol, d.Cod_Depto, d.Nome_Depto ,
 f.Cod_Func, f.Nome_Func, p.Cod_Produto, p.Nome_produto,
  i.Qtde, i.Valor
 from solicitacoes s inner join  departamentos d on d.Cod_Depto = s.Cod_Depto inner join  funcionarios f on f.Cod_Func = s.Cod_Func inner join  itens_solicitacao i on i.Num_Sol = s.Num_Sol inner join  produtos p on p.Cod_Produto = i.Cod_Produto;
select * from vw_solicita order by  Num_Sol desc;